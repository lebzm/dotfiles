...
+ consider using mwim
+ consider using puni
+ consider using golden-ratio-scroll-screen
+ consider using beginend
...
#+begin_src elisp
  (use-package project
    ...
    ("SPC d d" project-dired :name "project directory")
    ("SPC d ." dired-jump :name "directory")
    ("SPC f f" project-find-file :name "project file")
...
#+begin_src elisp :tangle no :noweb-ref startup-script
  (progn
    (require 'ob-tangle)
...
#+begin_src elisp :tangle no :noweb-ref early-setup
...
#+begin_src elisp
  (use-package org-present
    :hook
    (org-present-mode . smooth/present-start)
    (org-present-mode-quit . smooth/present-stop)
    :config
    (defun smooth/present-start ()
      (org-present-big)
      (toggle-frame-fullscreen)
      (hide-mode-line-mode 1)
      (visual-line-fill-column-mode 1))
    (defun smooth/present-stop ()
      (org-present-small)
      (toggle-frame-fullscreen)
      (hide-mode-line-mode 0)
      (visual-line-fill-column-mode 0))
    (defun smooth/present-prepare-slide (buffer heading)
      (org-overview)
      (org-fold-show-entry)
      (org-fold-show-children))
    (add-hook 'org-present-after-navigate-functions 'smooth/present-prepare-slide))
#+end_src
